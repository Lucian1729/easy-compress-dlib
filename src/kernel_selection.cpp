...
std::size_t select_best_kernel(std::string_view file_type, double alpha, std::vector<KernelMetrics>& kernels) {
...
std::size_t select_best_kernel(std::string_view file_type, double alpha) {
    std::vector<KernelMetrics> emptyKernels;
    return select_best_kernel(file_type, alpha, emptyKernels);
}
